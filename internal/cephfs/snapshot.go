...
func createSnapshot(ctx context.Context, volOptions *volumeOptions, cr *util.Credentials, snapID, volID volumeID) error {
	...
	stdErr, err := execCommandWithStdErr(
		...
		args[:]...)
	if err != nil {
		util.ErrorLog(ctx, "failed to create subvolume snapshot %s %s in fs %s with Error: %v and stdError %v", string(snapID), string(volID), volOptions.FsName, err, stdErr)
		return err
	}
	return nil
}
...
func deleteSnapshot(ctx context.Context, volOptions *volumeOptions, cr *util.Credentials, snapID, volID volumeID) error {
	...
	stdErr, err := execCommandWithStdErr(
		...
		args[:]...)
	if err != nil {
		util.ErrorLog(ctx, "failed to delete subvolume snapshot %s %s in fs %s with Error: %v and stdError: %v", string(snapID), string(volID), volOptions.FsName, err, stdErr)
		return err
	}
	return nil
}
...
func getSnapshotInfo(ctx context.Context, volOptions *volumeOptions, cr *util.Credentials, snapID, volID volumeID) (snapshotInfo, error) {
	...
	if err != nil {
		...
		util.ErrorLog(ctx, "failed to get subvolume snapshot info %s %s in fs %s with Error: %v and stdError: %v", string(snapID), string(volID), volOptions.FsName, err, stdErr)
...
func protectSnapshot(ctx context.Context, volOptions *volumeOptions, cr *util.Credentials, snapID, volID volumeID) error {
	...
	if err != nil {
		...
		util.ErrorLog(ctx, "failed to protect subvolume snapshot %s %s in fs %s with Error: %v and stdError: %v", string(snapID), string(volID), volOptions.FsName, err, stdErr)
		return err
	}
	return nil
}
...
func unprotectSnapshot(ctx context.Context, volOptions *volumeOptions, cr *util.Credentials, snapID, volID volumeID) error {
	...
	if err != nil {
		...
		util.ErrorLog(ctx, "failed to unprotect subvolume snapshot %s %s in fs %s with Error: %v and stdError: %v", string(snapID), string(volID), volOptions.FsName, err, stdErr)
		return err
	}
	return nil
}
...
func cloneSnapshot(ctx context.Context, parentVolOptions *volumeOptions, cr *util.Credentials, volID, snapID, cloneID volumeID, cloneVolOptions *volumeOptions) error {
	...
	if err != nil {
		util.ErrorLog(ctx, "failed to clone subvolume snapshot %s %s in fs %s with Error: %v and stdError: %v", string(cloneID), string(volID), parentVolOptions.FsName, err, stdErr)
