...
func (cs *ControllerServer) createBackingVolume(
	...
	var err error
	volClient := core.NewVolume(volOptions.Conn, &volOptions.Volume, volOptions.ClusterID)
	...
	if sID != nil {
		...
		snap := core.CephfsSnapshot{
			SnapshotID: sID.SnapshotID,
			Volume:     &parentVolOpt.Volume,
		}
...
func (cs *ControllerServer) CreateVolume(
	...
	if vID != nil {
		if sID != nil || pvID != nil {
			// volClinet:=core.NewVolume(conn *util.ClusterConnection, vol *core.Volume, fscID int64, clusterID string)
			volClinet := core.NewVolume(volOptions.Conn, &volOptions.Volume, volOptions.ClusterID)
			err = volClinet.ExpandVolume(ctx, volOptions.Size)
			if err != nil {
				purgeErr := volClinet.PurgeVolume(ctx, false)
				if purgeErr != nil {
	...
	// volClient:=core.NewVolume(conn *util.ClusterConnection, vol *core.Volume, fscID int64, clusterID string)
	volClient := core.NewVolume(volOptions.Conn, &volOptions.Volume, volOptions.ClusterID)
...
func (cs *ControllerServer) CreateSnapshot(
	...
	// too.
	volClient := core.NewVolume(parentVolOptions.Conn,
		&parentVolOptions.Volume,
		parentVolOptions.ClusterID)
	...
	if sid != nil {
		...
		if !(snapInfo.Protected == core.SnapshotIsProtected) {
			snapClient := core.NewSnapshot(parentVolOptions.Conn, sid.FsSnapshotName, &parentVolOptions.Volume)
...
func doSnapshot(
	...
	snapClient := core.NewSnapshot(volOpt.Conn, snapshotName, &volOpt.Volume)
...
func (cs *ControllerServer) DeleteSnapshot(
	...
	snapClient := core.NewSnapshot(volOpt.Conn, sid.FsSnapshotName, &volOpt.Volume)
	if snapInfo.Protected == core.SnapshotIsProtected {
