...
func (ri *rbdImage) addSnapshotScheduling(ctx context.Context,
	...
	for _, schedule := range schedules {
		log.DebugLog(ctx, "found snapshot schedle for image %q: schedule level %v name %v", ri, schedule.LevelSpecID, schedule.Name)
		for _, spec := range schedule.Schedule {
			log.DebugLog(ctx, "found snapshot schedle for image %q: schedule %v %v", ri, spec.Interval, spec.StartTime)
